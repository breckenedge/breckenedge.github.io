...
This is the piece that moved the needle the most. New project, legacy project, doesn't matter — clone, open in VS Code, and the devcontainer comes up with Ruby, Node, Postgres, Redis, whatever the project needs. Zero setup on the host. I used to spend hours getting a legacy Rails app running on a new machine. Now it's minutes.

The `devcontainer.json` does the heavy lifting: base image, system dependencies, `bundle install`, `yarn install`, background services. If you've never used them, they're worth the hour it takes to learn.

## Why I'm not using git worktrees

I tried. The problem is specific to Rails with Shakapacker. The Shakapacker dev server runs on its own port, and that port gets baked into the client-side HTML at build time so the browser knows where to fetch assets from. Rails has to know about it at startup.
...
The trick is a `CLAUDE.md` in the project root that tells it how to reach in:

```md
Commands need to run inside the devcontainer. Use `docker exec` to run them:
...
The thing I'd like to change eventually is running Claude inside the devcontainer instead. If it's already in the container, I can pass `--dangerously-skip-permissions` and stop worrying about the `docker exec` indirection entirely. I haven't gotten that working yet — the MCP server access is the sticking point — but it's on the list.
...
- **Jira and Confluence** — tickets, epics, internal docs
- **Sentry** — error tracking. I can point Claude at a specific error and it can look at the stack trace and the relevant code at the same time.
- **CircleCI** — build and pipeline status
- **Figma** — design specs
...
So there's a `bin/mcp-setup` script in each repo. You clone, you run it, it presents a menu of supported agents, and it writes the right config to the right place for whichever one you picked. That's it. The list of MCP servers is defined per-project inside the script, so different repos wire up different tools depending on what's relevant.
...
Skills are another piece worth mentioning. They're slash commands you can add to Claude Code — reusable prompts that expand when you invoke them. I've got one for `/gmail` that searches my inbox using the Gmail Python API.
...
Open a project, devcontainer comes up (or it's already running), open Claude Code in a terminal on the host. Give it a task. It reads the `CLAUDE.md`, knows how to run things in the container, pulls what it needs from Jira or Sentry or GitHub, and gets to work.
