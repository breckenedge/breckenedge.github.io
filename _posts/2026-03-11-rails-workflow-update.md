...
I also haven't tried the [Ralph Loop](https://github.com/snarktank/ralph) yet — the pattern of running an agent repeatedly against a PRD until it's done, with git as the memory between fresh sessions. It's been on my radar. But Claude Code just shipped [`/loop`](https://code.claude.com/docs/en/scheduled-tasks), a native scheduled task command, and I think that changes the calculus. `/loop` isn't Ralph — it's for recurring tasks on a schedule, not driving toward a goal. It's a better fit for what [OpenClaw](https://en.wikipedia.org/wiki/OpenClaw) was doing: always-on background work triggered on a timer. The weekly radar update I open an issue for manually today could just be a `/loop`. I'll probably get there before I get to Ralph.
